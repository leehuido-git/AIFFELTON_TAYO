...
void twi_init(void)
{
  ...
  /* twi bit rate formula from atmega2560 manual pg 248
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  	uint8_t i;
#ifdef TWI_DEBUG
	printf("In twi_readFrom(%x, %x, %x, %x)\n\r" , address , data , length , sendStop);
#endif
  	// ensure data will fit into buffer
  	if(TWI_BUFFER_LENGTH < length)
	{
#ifdef TWI_DEBUG
		printf("Buffer_Length (0x%x) < length (0x%x) -> Wont fit...\n\r" , TWI_BUFFER_LENGTH , length);
#endif
    		return 0;
  	}

#ifdef TWI_DEBUG
	printf("wait until twi is ready, become master receiver");
#endif
...
#ifdef TWI_DEBUG
	printf("Will read from ddress %x\n\r" , twi_slarw);
#endif
...
#ifdef TWI_DEBUG
  	printf("Wait until twi is ready, become master transmitter");
#endif
...
#ifdef TWI_DEBUG
  	printf("Will write to address %x\n\r" , twi_slarw);
#endif
...
#ifdef TWI_DEBUG
  	printf("Wait for write operation to complete");
#endif
